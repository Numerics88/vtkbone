...
// .NAME vtkboneApplySymmetricShearTest - Generates a finite element mesh that
...
public:
  ...
  // Description:
  // Sets the amount of apparant engineering shear strain.
  // In this context it is "apparant" because it is taken over the test
  // volume as a whole.
  // Default is 0.01 .
  ...
  // Description:
  // If True, restrict nodes on the side faces from moving in the z direction
  // (in the Test Frame).  Default is True.
  vtkSetMacro(ConfineSidesVertically, int);
  vtkGetMacro(ConfineSidesVertically, int);
  vtkBooleanMacro(ConfineSidesVertically, int);

  // Description:
  // If True, restrict nodes on the top and bottom faces from moving in the z direction
  // (in the Test Frame).  Default is True.

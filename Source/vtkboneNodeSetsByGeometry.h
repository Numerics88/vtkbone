...
// .NAME vtkboneNodeSetsByGeometry - various algorithms to select nodes sets
...
public:
  ...
  // lie on the defined plane.  The plane is defined by two parameters: axis
  // and val. The plane intersects the coordinate axis given by axis (=0,1,2)
  // at val (a float). For example, to find all nodes lying on the bottom Z
  // face of a cube, set axis=2 and val = 0.0 (or whatever the minimum Z value
  // is in the data set).
  ...
  // Description:
  // Calls FindNodesIntersectingTwoPlanes and adds the resulting node set to
  // model.
  ...
  // Description:
  // Calls FindNodesIntersectingThreePlanes and adds the resulting node set to
  // model.
  ...
  // Description:
  // Function returns the node ids of all points that you would be able to see
  // if you looked down upon the object from the specified dirction.
  // This is useful for specifying which nodes
  // should have forces applied to them if the surface is uneven.
  // normalVector is the vector from the object towards the viewer.
  // If the input object contains a PedigreeIds array, those values will be
  // returned in visibleNodesIds; otherwise the point ids of the input
  // object will be used.

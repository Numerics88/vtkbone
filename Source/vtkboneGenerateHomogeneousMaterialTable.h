...
/*! @class   vtkboneGenerateHomogeneousMaterialTable
    @brief   generates a
...
public:
  ...
  //@{
  /*! Set/Get the first index to use in the table. Default 1. */
  vtkSetMacro(FirstIndex, int);
  vtkGetMacro(FirstIndex, int);
  //@}

  //@{
  /*! Set/Get the last index to use in the table. Default 127. */
  vtkSetMacro(LastIndex, int);
  vtkGetMacro(LastIndex, int);
  //@}

  //@{
  /*! Pass a list of Ids to construct table entries for.  If set, then
      FirstIndex and LastIndex are ignored. Default is NULL. */

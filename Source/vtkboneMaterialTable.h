...
/*! @class   vtkboneMaterialTable
    @brief   Material Table finite element mesh
...
class VTKBONE_EXPORT vtkboneMaterialTable : public vtkDataObject
{
  public:
    ...
    //@{
    /*! Set an entry in the table. If the entry exists already, will
	replace it. */
	...
	the material. Returns the assigned index. */
    int AppendMaterial(vtkboneMaterial* material);

    /*! Removes an entry in the table. If the entry does not exist,
	...
	with the largest index less than specified, together with the
	difference in indices (the offset). In the case of a non-zero
    ...
    /*! This returns the material at index if it exists, or else material
	with the largest index less than specified, together with the
	difference in indices, obtained by an immediately following call to
	GetArrayOffset. In this case it is your responsibility to check
	(via a dynamic cast), that the returned material is in fact a
	material array if offset in non-zero. This version is for use in
	Python. */
    vtkboneMaterial* GetMaterialOrArray(int index);
    int GetArrayOffset ();
    //@}

    /*! Initialize the traversal of the table. This means the data pointer
	is set at the beginning of the list. This method is not
	thread-safe. */
    void InitTraversal();

    /*! Get the next item in the table. NULL is returned if the table is
	exhausted. Note that immediately after a call to InitTraversal,
	this returns the first item in the list. This method is not
	thread-safe. */
    int GetNextIndex();

    /*! Get the next item in the table, that has not already been returned.
	NULL is returned if the table is exhausted. Note that immediately
	after a call to InitTraversal, this returns the first item in the
	list. Note that you cannot mix calls to GetNextIndex with calls to
	GetNextUniqueIndex: the results are undetermined. This method is
	not thread-safe. */
    int GetNextUniqueIndex();

    /*! Get the current material in the traversal. This method is only
	valid after a call to GetNextIndex()! */
    vtkboneMaterial* GetCurrentMaterial();

    /*! Verifies that all the materials have unique names. Returns 1 if all
	materials have unique names, 0 otherwise. Also returns 0 if any
	material does not have a name. */
    int CheckNames();

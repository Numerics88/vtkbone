...
/*! @class   vtkboneApplyDirectionalShearTest
...
public:
  ...
  //@{
  /*! Sets the amount of shear as a vector in the x,y plane (in the Test
      Frame). Units are length units if ScaleShearToHeight is False,
      otherwise a unitless ratio.  Default is (0.01,0) . */
  vtkSetVector2Macro(ShearVector, double);
  vtkGetVector2Macro(ShearVector, double);
  //@}

  //@{
  /*! If True, the ShearVector will be multiplied by the height of the
      model extent (in the z direction in the Test Frame) to determine the

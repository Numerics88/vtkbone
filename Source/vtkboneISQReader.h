...
/*! @class   vtkboneISQReader
    @brief   read Scanco .isq files.

 vtkboneISQReader is a source object that reads Scanco .isq files.

 vtkboneISQReader creates vtkImageData datasets.
...
#ifndef __vtkboneISQReader_h
#define __vtkboneISQReader_h
...
class VTKBONE_EXPORT vtkboneISQReader : public vtkImageAlgorithm
{
public:
  static vtkboneISQReader *New();
  vtkTypeMacro(vtkboneISQReader, vtkImageAlgorithm);
  ...
  //@{
  /*! Get ISQ Element Size.  Units are mm. */
...
private:
  ...
  vtkboneISQReader(const vtkboneISQReader&);
  void operator=(const vtkboneISQReader&);
};

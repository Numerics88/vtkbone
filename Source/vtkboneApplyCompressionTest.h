...
/*! @class   vtkboneApplyCompressionTest
...
public:
  ...
  //@{
  /*! Set/get an applied apparent level strain. Negative for compression,
      positive for tension. Default is -0.01 (corresponds to -1% apparent
      level strain). */
  ...
  //@{
  /*! Set/get the absolute displacement applied to the mesh. If zero
      (default), it will be derived from the AppliedStrain and the model
      dimensions. */
  ...
  //@{
  /*! Prevents the side surfaces (face_x0, face_x1, face_y0, face_y1) from
      moving in the x or y directions in the Test Frame. Default is off. */
  ...
  /*! Set whether to add a pin that fully defines a model.  This pin
      ...
      uniaxial models are singular (although n88solver can still solve
      them). If any of TopSurfaceContactFriction,
      BottomSurfaceContactFriction or ConfineSides is True, then this
      option is ignored, as it is then no longer relevant Default is off. */
  vtkSetMacro(Pin, int);
  vtkGetMacro(Pin, int);
  ...
  //@{
  /*! Set/get the Cell Id of the cell to be pinned.  Only applies if a pin
      is used. Default is 0.  Note that if PinCellClosestToXYCenter, this
      value will be changed on Update. */
  vtkSetMacro(PinCellId, vtkIdType);
  vtkGetMacro(PinCellId, vtkIdType);
  //@}

  //@{
  /*! Set whether to locate the pinned element closest to the x,y center
      (in the Test Frame).  If False, then the existing PinCellId is used.
      It takes some computational time to find the cell closest to the
      center. Where 2 elements are equidistant from the center, the one
      with smallest z coordinate will be chosen. Default is on. */

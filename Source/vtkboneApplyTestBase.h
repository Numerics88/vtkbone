...
/*! @class   vtkboneApplyTestBase
    @brief   Base class for applying a test to create a
...
public:
  ...
  //@{
  /*! Limit the applied boundary conditions on the top surface to the
      specified material. -1 indicates any material (default). */
  vtkSetMacro(TopConstraintSpecificMaterial, int);
  vtkGetMacro(TopConstraintSpecificMaterial, int);
  //@}

  //@{
  /*! Limit the applied boundary conditions on the bottom surface to the
      specified material. -1 indicates any material (default). */
  ...
  //@{
  /*! In cases where the surface to which force or displacement boundary
      conditions are on an uneven surface, setting this option on will
      search for all nodes on the uneven surface. The default, without this
      setting turned on, will only find nodes that intersect the plane
      defined by the WholeExtent. Default is off. */
  ...
  //@{
  /*! Flag to set limit to depth of top surface, as measured from top
      boundary of volume.  Only applies if UnevenTopSurface is true.
      Default is off. */
  ...
  //@{
  /*! Flag to set limit to depth of bottom surface, as measured from bottom
      boundary of volume.  Only applies if UnevenBottomSurface is true.
      Default is off. */
  ...
  //@{
  /*! Sets the maximum depth of the bottom surface if UnevenBottomSurface
      and UseBottomSurfaceMaximumDepth are true. */
  ...
  //@{
  /*! Set/Get the testing axis (x=0, y=1, z=2) Default is 2. */
  ...
  /*! Given an sense (i.e. an axis direction) in the Test Frame and a
      polarity (0=negative, 1=positive), return the polarity flag in the
      Data Frame. */
  int DataFramePolarity(int testFrameSense, int polarity);

  /*! Given bounds in the Data Frame, a sense (i.e. an axis direction) in
      the Test Frame and a polarity in the Test Frame (0=negative,
      1=postive), return the value of the indicated bound. */
  double TestFrameBound(double bounds[6], int testFrameSense, int polarity);

  /*! Given a VTK local point Id for a voxel in the Test Frame (in the
      range 0 to 7), return the point Id in the Data Frame. */
...
protected:
  ...
  /*! Add Sets.  The default behaviour of this method is to call
      ...
      override this behaviour if they require different node and element
      sets. */
  virtual int AddSets(vtkboneFiniteElementModel* model);

  //@{
  /*! Add node and element sets on particular faces. These methods may be
      overridden by child classes if they require unusual methods of
      selecting the nodes. */
